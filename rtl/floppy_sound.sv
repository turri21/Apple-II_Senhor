...
module floppy_sound #(
	parameter integer MOTOR_NOISE_DIVIDER = 1024,
	parameter integer MOTOR_ENVELOPE_DIVIDER = 477273,
	parameter integer SPINDLE_HALF_PERIOD = 1431818,
	parameter integer STEP_DECAY_DIVIDER = 512,
	parameter integer STEP_RING_DECAY_DIVIDER = 4096,
	parameter integer IO_DECAY_DIVIDER = 512,
	parameter integer IO_RETRIGGER_CYCLES = 4096,
	parameter integer MOTOR_LEVEL = 6,
	parameter integer STEP_LEVEL_SHIFT = 2,
	parameter integer STEP_RING_LEVEL_SHIFT = 1,
...
wire [9:0] drive1_step_sample = {6'd0, drive1_step_envelope} << STEP_LEVEL_SHIFT;
wire [9:0] drive2_step_sample = {6'd0, drive2_step_envelope} << STEP_LEVEL_SHIFT;
wire [9:0] drive1_step_ring_sample = noise_lfsr[4] ?
	({7'd0, drive1_step_ring} << STEP_RING_LEVEL_SHIFT) : 10'd0;
wire [9:0] drive2_step_ring_sample = noise_lfsr[5] ?
	({7'd0, drive2_step_ring} << STEP_RING_LEVEL_SHIFT) : 10'd0;
wire [9:0] drive1_io_sample = noise_lfsr[2] ? ({7'd0, drive1_io_envelope} << IO_LEVEL_SHIFT) : 10'd0;
wire [9:0] drive2_io_sample = noise_lfsr[3] ? ({7'd0, drive2_io_envelope} << IO_LEVEL_SHIFT) : 10'd0;
wire [10:0] mixed_sample = {1'b0, motor_sample} +
...
always @(posedge clk) begin
	if(reset || !enable) begin
		...
		end else if(drive1_step_ring || drive2_step_ring) begin
